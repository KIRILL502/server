...
function aAdminMenu ()
	if ( aAdminForm == nil ) then
		...
		aAdminForm			= guiCreateWindow ( x / 2 - 310, y / 2 - 260, 620, 520, "", false )
						  ...
						  guiSetText ( aAdminForm, "Панель админа  -   v".._version )
						  guiCreateLabel ( 0.75, 0.05, 0.45, 0.04, "Гл. Админ: Jumper", true, aAdminForm )
		...
		aTab6.PasswordChange	= guiCreateButton ( 0.85, 0.35, 0.10, 0.04, "Принять", true, aTab6.Tab )
						  guiCreateHeader ( 0.03, 0.65, 0.20, 0.055, "Настройки:", true, aTab6.Tab )
						  guiCreateStaticImage ( 0.03, 0.69, 0.94, 0.0025, "client\\images\\dot.png", true, aTab6.Tab )
						  guiCreateLabel ( 0.03, 0.71, 0.22, 0.055, "Приоритет исполнения:", true, aTab6.Tab )
						  guiCreateLabel ( 0.11, 0.76, 0.10, 0.05, "Память", true, aTab6.Tab )
						  guiCreateLabel ( 0.11, 0.81, 0.10, 0.05, "Авто", true, aTab6.Tab )
						  guiCreateLabel ( 0.11, 0.86, 0.10, 0.05, "Скорость", true, aTab6.Tab )
		...
		aTab6.PerformanceAdvanced = guiCreateButton ( 0.05, 0.91, 0.11, 0.04, "Обновить", true, aTab6.Tab )
		aPerformance()
					   	  guiCreateLabel ( 0.67, 0.90, 0.21, 0.055, "Задержка обновления(мс):", true, aTab6.Tab )
...
function aAdminRefresh ()
	if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) ~= -1 ) then
		...
		if ( player and aPlayers[player] ) then
			guiSetText ( aTab1.Name, "Ник: "..aPlayers[player]["name"] )
			guiSetText ( aTab1.Mute, iif ( aPlayers[player]["mute"], "Анмут", "Мут" ) )
			guiSetText ( aTab1.Freeze, iif ( aPlayers[player]["freeze"], "Анфриз", "Фриз" ) )
			--guiSetText ( aTab1.Username, "Community Username: "..( aPlayers[player]["username"] or "" ) )
			guiSetText ( aTab1.Version, "Версия: "..( aPlayers[player]["version"] or "" ) )
			guiSetText ( aTab1.Accountname, "Аккаунт: "..( aPlayers[player]["accountname"] or "" ) )
			guiSetText ( aTab1.Groups, "Группа: "..( aPlayers[player]["groups"] or "нет" ) )
			if ( isPedDead ( player ) ) then guiSetText ( aTab1.Health, "Здоровье: умер" )
			...
			if ( getPlayerTeam ( player ) ) then guiSetText ( aTab1.Team, "Команда: "..getTeamName ( getPlayerTeam ( player ) ) )
			else guiSetText ( aTab1.Team, "Команда: нет" ) end
			...
			if ( getElementInterior ( player ) ) then guiSetText ( aTab1.Interior, "Интерьер: "..getElementInterior ( player ) ) end
			guiSetText ( aTab1.JetPack, iif ( doesPedHaveJetPack ( player ), "Отнять джетпак", "Дать джетпак" ) )
			...
			guiSetText ( aTab1.Area, "Местность: "..iif ( getZoneName ( x, y, z, false ) == getZoneName ( x, y, z, true ), getZoneName ( x, y, z, false ), getZoneName ( x, y, z, false ).." ("..getZoneName ( x, y, z, true )..")" ) )
			guiSetText ( aTab1.PositionX, "X: "..x )
			guiSetText ( aTab1.PositionY, "Y: "..y )
			guiSetText ( aTab1.PositionZ, "Z: "..z )
			local vehicle = getPedOccupiedVehicle ( player )
			if ( vehicle ) then
				guiSetText ( aTab1.Vehicle, "Автомобиль: "..getVehicleName ( vehicle ).." (ID: "..getElementModel ( vehicle )..")" )
				guiSetText ( aTab1.VehicleHealth, "Здоровье автомобиля: "..math.ceil ( getElementHealth ( vehicle ) ).."%" )
			else
				guiSetText ( aTab1.Vehicle, "Автомобиль: Foot" )
				guiSetText ( aTab1.VehicleHealth, "Здоровье автомобиля: 0%" )
			end
			if ( aPlayers[player]["admin"] ) then
				guiSetText(aTab1.Admin, "Отнять админские права")
			else
				guiSetText(aTab1.Admin, "Дать админские права")
			end
			return player
		end
	end
...
function aClientSync ( type, table )
	...
	elseif ( type == "server" ) then
		guiSetText ( aTab3.Server, "Сервер: "..table["name"] )
		guiSetText ( aTab3.Players, "Игроки: "..#getElementsByType ( "player" ).."/"..table["players"] )
		guiSetText ( aTab3.Password, "Пароль: "..( table["password"] or "нет" ) )
		guiSetText ( aTab3.GameType, "Мод: "..( table["game"] or "нет" ) )
		guiSetText ( aTab3.MapName, "Карта: "..( table["map"] or "нет" ) )
	...
	elseif ( type == "bans" or type == "bansmore" ) then
		...
		guiSetText( aTab4.BansTotal, "Показ  " .. amount .. " / " .. total .. "  банов" )
	...
	elseif ( type == "messages" ) then
		...
		guiSetText ( aTab1.Messages, table["unread"].."/"..table["total"].." непрочитанные сообщения" )
	end
end
...
function aClientResourceStart ( resource )
	...
	while ( id <= guiGridListGetRowCount( aTab2.ResourceList ) ) do
		if ( guiGridListGetItemText ( aTab2.ResourceList, id, 1 ) == resource ) then
			guiGridListSetItemText ( aTab2.ResourceList, id, 3, "запущен", false, false )
		end
		id = id + 1
	end
end
...
function aClientResourceStop ( resource )
	...
	while ( id <= guiGridListGetRowCount( aTab2.ResourceList ) ) do
		if ( guiGridListGetItemText ( aTab2.ResourceList, id, 1 ) == resource ) then
			guiGridListSetItemText ( aTab2.ResourceList, id, 3, "загружен", false, false )
		end
		id = id + 1
	end
end
...
function aSetCurrentAmmo ( ammo )
	ammo = tonumber ( ammo )
	if ( ( ammo ) and ( ammo > 0 ) and ( ammo < 100010000 ) ) then
		aCurrentAmmo = ammo
		return
	end
	...
	if ( button == "left" ) then
		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( getElementType ( source ) == "gui-button" )  then
				...
				if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) == -1 ) then
					aMessageBox ( "Ошибка", "Игрок не выбран!" )
				else
					...
					if ( source == aTab1.Kick ) then aInputBox ( "Кик игрока "..name, "Ввод причины кика", "", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"kick\", $value )" )
					...
					elseif ( source == aTab1.Mute ) then if not aPlayers[player]["mute"] then aMuteInputBox ( player ) else aMessageBox ( "Вопрос", "Точно хотите разоткнуть "..name.."?", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"mute\" )" ) end
					...
					elseif ( source == aTab1.Nick ) then aInputBox ( "Поставить ник", "Ввод нового ника для игрока", name, "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"nick\", $value )" )
					elseif ( source == aTab1.Shout ) then aInputBox ( "Сказать", "Ввод текста для игрока", "", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"shout\", $value )" )
					elseif ( source == aTab1.SetHealth ) then aInputBox ( "Дать здоровье", "Ввод значения здоровья", "100", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"sethealth\", $value )" )
					elseif ( source == aTab1.SetArmour ) then aInputBox ( "Дать броню", "Ввод значения брони", "100", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"setarmour\", $value )" )
					...
					elseif ( source == aTab1.SetMoney ) then aInputBox ( "Дать денег", "Ввод значения денег", "0", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"setmoney\", $value )" )
					elseif ( source == aTab1.SetStats ) then aPlayerStats ( player )
					elseif ( source == aTab1.SetDimension ) then aInputBox ( "ID измерения", "Ввод ID измерения между 0  и 65535", "0", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"setdimension\", $value )" )
					...
					elseif ( source == aTab1.Admin ) then
						if ( aPlayers[player]["admin"] ) then aMessageBox ( "Внимание", "Отнять админские права у "..name.."?", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"setgroup\", false )" )
						else aMessageBox ( "Внимание", "Дать админские права "..name.."?", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"setgroup\", true )" ) end
					end
				end
			...
			elseif ( source == aTab1.PlayerList ) then
				if ( guiGridListGetSelectedItem( aTab1.PlayerList ) ~= -1 ) then
					...
					if ( player ) then
						...
						if ( ( guiCheckBoxGetSelected ( aTab6.OutputPlayer ) ) and ( player ) ) then outputConsole ( "Ник: "..aPlayers[player]["name"]..", IP: "..aPlayers[player]["IP"]..", Серийный номер: "..aPlayers[player]["serial"]..", Community Username: "..aPlayers[player]["username"]..", Аккаунт: "..aPlayers[player]["accountname"] ) end
						guiSetText ( aTab1.IP, "IP: "..aPlayers[player]["IP"] )
						guiSetText ( aTab1.Serial, "Серийный номер: "..aPlayers[player]["serial"] )
						--guiSetText ( aTab1.Username, "Community Username: "..aPlayers[player]["username"] )
						guiSetText ( aTab1.Accountname, "Аккаунт: "..aPlayers[player]["accountname"] )
					...
					guiSetText ( aTab1.Name, "Ник: N/A" )
					guiSetText ( aTab1.IP, "IP: N/A" )
					guiSetText ( aTab1.Serial, "Серийный номер: N/A" )
					--guiSetText ( aTab1.Username, "Community Username: N/A" )
					guiSetText ( aTab1.Version, "Версия: N/A" )
					guiSetText ( aTab1.Accountname, "Аккаунт: N/A" )
					guiSetText ( aTab1.Groups, "Группа: N/A" )
					guiSetText ( aTab1.Mute, "Мут" )
					guiSetText ( aTab1.Freeze, "Фриз" )
					guiSetText ( aTab1.Admin, "Дать админские права" )
					guiSetText ( aTab1.Health, "Здоровье: 0%" ) 
					guiSetText ( aTab1.Armour, "Броня: 0%" )
					guiSetText ( aTab1.Skin, "Скин: N/A" )
					guiSetText ( aTab1.Team, "Команда: нет" )
					guiSetText ( aTab1.Ping, "Пинг: 0" )
					guiSetText ( aTab1.Money, "Деньги: 0" )
					guiSetText ( aTab1.Dimension, "Измерение: 0" )
					guiSetText ( aTab1.Interior, "Интерьер: 0" )
					guiSetText ( aTab1.JetPack, "Дать джетпак" )
					guiSetText ( aTab1.Weapon, "Оружие: N/A" )
					guiSetText ( aTab1.Area, "Местность: Неизвестно" )
					guiSetText ( aTab1.PositionX, "X: 0" )
					guiSetText ( aTab1.PositionY, "Y: 0" )
					guiSetText ( aTab1.PositionZ, "Z: 0" )
					guiSetText ( aTab1.Vehicle, "Автомобиль: N/A" )
					guiSetText ( aTab1.VehicleHealth, "Здоровье автомобиля: 0%" )
		...
		elseif ( getElementParent ( source ) == aTab2.Tab ) then
			...
			elseif ( ( source == aTab2.ResourceStart ) or ( source == aTab2.ResourceRestart ) or ( source == aTab2.ResourceStop ) or ( source == aTab2.ResourceSettings ) ) then
				if ( guiGridListGetSelectedItem ( aTab2.ResourceList ) == -1 ) then
					aMessageBox ( "Ошибка", "Ресурс не выбран!" )
				else
		...
		elseif ( getElementParent ( source ) == aTab3.Tab ) then
			if ( source == aTab3.SetGameType ) then aInputBox ( "Мод", "Ввод названия мода:", "", "triggerServerEvent ( \"aServer\", getLocalPlayer(), \"setgame\", $value )" )
			elseif ( source == aTab3.SetMapName ) then aInputBox ( "Карта", "Ввод названия карты:", "", "triggerServerEvent ( \"aServer\", getLocalPlayer(), \"setmap\", $value )" )
			elseif ( source == aTab3.SetWelcome ) then aInputBox ( "Приветствие", "Ввод приветствия:", "", "triggerServerEvent ( \"aServer\", getLocalPlayer(), \"setwelcome\", $value )" )
			elseif ( source == aTab3.SetPassword ) then aInputBox ( "Пароль", "Пароль от сервера: (максимум 32 символа)", "", "triggerServerEvent ( \"aServer\", getLocalPlayer(), \"setpassword\", $value )" )
		...
		elseif ( getElementParent ( source ) == aTab4.Tab ) then
			if ( source == aTab4.Details ) then
				if ( guiGridListGetSelectedItem ( aTab4.BansList ) == -1 ) then
					aMessageBox ( "Ошибка", "Строка не выбрана!" )
				else
			...
			elseif ( source == aTab4.Unban ) then
				if ( guiGridListGetSelectedItem ( aTab4.BansList ) == -1 ) then
					...
					local selip = guiGridListGetItemText ( aTab4.BansList, guiGridListGetSelectedItem( aTab4.BansList ), 2 )
					local selserial = guiGridListGetItemText ( aTab4.BansList, guiGridListGetSelectedItem( aTab4.BansList ), 3 )
					if ( aBans["Serial"][selserial] ) then aMessageBox ( "Вопрос", "Разбан "..selserial.."?", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"unbanserial\", \""..selserial.."\" )" )
					else aMessageBox ( "Вопрос", "Разбан IP "..selip.."?", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"unbanip\", \""..selip.."\" )" ) end
				end
			elseif ( source == aTab4.UnbanIP ) then
				aInputBox ( "Разбан по IP", "Ввод IP для разбана", "", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"unbanip\", $value )" )
			elseif ( source == aTab4.UnbanSerial ) then
				aInputBox ( "Разбан по серийному номеру", "Ввод серийного номера для разбана", "", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"unbanserial\", $value )" )
			elseif ( source == aTab4.BanIP ) then
				aInputBox ( "Бан по IP", "Ввод IP для бана", "", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"banip\", $value )" )
			elseif ( source == aTab4.BanSerial ) then
				aInputBox ( "Бан серийного номера", "Ввод серийного номера для бана", "", "triggerServerEvent ( \"aBans\", getLocalPlayer(), \"banserial\", $value )" )
		...
		elseif ( getElementParent ( source ) == aTab6.Tab ) then
			...
			elseif ( source == aTab6.PasswordChange ) then
				...
				if ( passwordNew == "" ) then aMessageBox ( "Ошибка", "Ввод нового пароля" )
				elseif ( passwordConf == "" ) then aMessageBox ( "Ошибка", "Повтор нового пароля" )
				elseif ( string.len ( passwordNew ) < 4 ) then aMessageBox ( "Ошибка", "Новый пароль должен быть длинее 4 символов" )
				elseif ( passwordNew ~= passwordConf ) then aMessageBox ( "Ошибка", "Confirmed password doesn't match" )
...
function aClientRender ()
	if ( guiGetVisible ( aAdminForm ) ) then
		if ( getTickCount() >= aLastCheck ) then
			aAdminRefresh ()
			local th, tm = getTime()
			guiSetText ( aTab3.Players, "Игроки: "..#getElementsByType ( "player" ).."/"..gettok ( guiGetText ( aTab3.Players ), 2, 47 ) )
			guiSetText ( aTab3.TimeCurrent,	string.format("Время: %02d:%02d", th, tm ) )
			guiSetText ( aTab3.GravityCurrent, "Гравитация: "..string.sub ( getGravity(), 0, 6 ) )
			guiSetText ( aTab3.SpeedCurrent, "Скорость игры: "..getGameSpeed() )
			guiSetText ( aTab3.WeatherCurrent, "Погода: "..getWeather().." ("..getWeatherNameFromID ( getWeather() )..")" )
