...
function aSpectate ( player )
	if ( player == getLocalPlayer() ) then
		aMessageBox ( "Ошибка", "Невозможно следить за собой" )
		return
	end
...
function aSpectator.Initialize ()
	if ( aSpectator.Actions == nil ) then
		...
		aSpectator.Actions		= guiCreateWindow ( x - 190, y / 2 - 200, 160, 400, "Действия", false )
		aSpectator.Ban			= guiCreateButton ( 0.10, 0.09, 0.80, 0.05, "Бан", true, aSpectator.Actions )
		aSpectator.Kick			= guiCreateButton ( 0.10, 0.15, 0.80, 0.05, "Кик", true, aSpectator.Actions )
		aSpectator.Freeze			= guiCreateButton ( 0.10, 0.21, 0.80, 0.05, "Фриз", true, aSpectator.Actions )
		aSpectator.SetSkin		= guiCreateButton ( 0.10, 0.27, 0.80, 0.05, "Установить скин", true, aSpectator.Actions )
		aSpectator.SetHealth		= guiCreateButton ( 0.10, 0.33, 0.80, 0.05, "Дать здоровья", true, aSpectator.Actions )
		aSpectator.SetArmour		= guiCreateButton ( 0.10, 0.39, 0.80, 0.05, "Дать броню", true, aSpectator.Actions )
		aSpectator.SetStats		= guiCreateButton ( 0.10, 0.45, 0.80, 0.05, "Статистика", true, aSpectator.Actions )
		aSpectator.Slap			= guiCreateButton ( 0.10, 0.51, 0.80, 0.05, "Пнуть! "..aCurrentSlap.."hp", true, aSpectator.Actions )
		...
		aSpectator.Skip			= guiCreateCheckBox ( 0.08, 0.8, 0.84, 0.07, "Пропуск умерших", true, true, aSpectator.Actions )
					  		  guiCreateLabel ( 0.08, 0.89, 0.84, 0.04, "____________________", true, aSpectator.Actions )
		aSpectator.Back			= guiCreateButton ( 0.10, 0.93, 0.80, 0.05, "Назад", true, aSpectator.Actions )
	
		aSpectator.Players		= guiCreateWindow ( 30, y / 2 - 200, 160, 400, "Игроки", false )
		...
		aSpectator.PlayerList		= guiCreateGridList ( 0.03, 0.07, 0.94, 0.92, true, aSpectator.Players )
					  		  guiGridListAddColumn( aSpectator.PlayerList, "Ник игрока", 0.85 )
		...
		aSpectator.Prev			= guiCreateButton ( x / 2 - 100, y - 50, 95, 30, "< Предыдущий", false )
		aSpectator.Next			= guiCreateButton ( x / 2 + 30,  y - 50, 90, 30, "Следующий >", false )
...
function aSpectator.ClientDoubleClick ( button )
	if ( source == aSpectator.Slaps ) then
		if ( guiGridListGetSelectedItem ( aSpectator.Slaps ) ~= -1 ) then
			...
			guiSetText ( aTab1.Slap, "Пнуть! "..aCurrentSlap.."hp" )
			guiSetText ( aSpectator.Slap, "Пнуть! "..aCurrentSlap.."hp" )
		end
		guiSetVisible ( aSpectator.Slaps, false )
	end
end
...
function aSpectator.SwitchPlayer ( inc, arg, inc2 )
	...
	if ( #players <= 0 ) then
		aMessageBox ( "Вопрос", "Не за кем следить, выйти?", "aSpectator.Close ( false )" )
		return
	end
	local current = 1
...
function aSpectator.Render ()
	...
	if ( not aSpectator.Spectating ) then
		dxDrawText ( "Никто не смотрит", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	if ( not x ) then
		dxDrawText ( "Ошибка при получениии координат", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
