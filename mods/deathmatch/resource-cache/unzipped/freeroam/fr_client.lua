...
wndSkin = {
	...
	controls = {
		...
		{'txt', id='skinid', text='', width=73},
		{'btn', id='set', text='выбор', width=73, onclick=applySkin},
		{'btn', id='close', text='закрыть', width=73, closeswindow=true}
	},
	oncreate = skinInit
}
	...
	controls = {
		{
			...
			height=290,
			columns={
				{text='Анимация', attr='name'}
			},
			rows={xml='animations.xml', attrs={'name'}},
			expandlastlevel=false,
			onitemdoubleclick=applyAnimation
		},
		{'btn', id='set', text='выбрать', width=73, onclick=applyAnimation},
		{'btn', id='stop', text='стоп', width=73, onclick=stopAnimation},
		{'btn', id='close', text='закрыть', width=73, closeswindow=true}
	}
}
	...
	controls = {
		{
			...
			height=280,
			columns={
				{text='Оружие', attr='name'}
			},
			rows={xml='weapons.xml', attrs={'id', 'name'}},
			onitemdoubleclick=function(leaf) addWeapon(leaf, 500) end
		},
		{'br'},
		{'txt', id='amount', text='500', width=73},
		{'btn', id='add', text='взять', width=73, onclick=addWeapon},
		{'btn', id='close', text='закрыть', width=73, closeswindow=true}
	}
}
...
function clothesInit()
	if getElementModel(g_Me) ~= 0 then
		errMsg("У вас должен быть скин CJ'я для того, чтобы одевать одежду.")
...
function clothListClick(cloth)
	setControlText(wndClothes, 'addremove', cloth.wearing and 'снять' or 'одеть')
end
...
function applyClothes(cloth)
	...
	if cloth.wearing then
		cloth.wearing = false
		setControlText(wndClothes, 'addremove', 'одеть')
		...
		cloth.wearing = true
		setControlText(wndClothes, 'addremove', 'снять')
	...
	controls = {
		{
			...
			height=390,
			columns={
				{text='Одежда', attr='name', width=0.6},
				{text='Одето', attr='wearing', enablemodify=true, width=0.3}
			},
			rows={
				{name='Запрос списка одежды...'}
			},
			onitemclick=clothListClick,
			onitemdoubleclick=applyClothes
		},
		{'br'},
		{'btn', id='addremove', text='одеть', width=70, onclick=applyClothes},
		{'btn', id='close', text='закрыть', width=70, closeswindow=true}
	},
	...
	controls = {
		{
			...
			height=200,
			columns={
				{text='Гравитация', attr='name'}
			},
			rows={
				{name='Космическая', value=0},
				{name='Лунная', value=0.001},
				{name='Нормальная', value=0.008},
				{name='Сильная', value=0.015}
			},
			onitemclick=selectPlayerGrav,
			onitemdoubleclick=applyPlayerGrav
		},
		{'lbl', text='Точное значение:', width=110},
		{'txt', id='gravval', text='', width=84},
		{'br'},
		{'btn', id='ok', text='применить', width=97, onclick=applyPlayerGrav},
		{'btn', id='cancel', text='закрыть', width=97, closeswindow=true}
	},
...
wndWarp = {
	'wnd',
	text = 'Телепортирование к игроку',
	width = 300,
	controls = {
		{
			'lst',
			...
			height=330,
			columns={
				{text='Никнейм', attr='name'}
			},
			onitemdoubleclick=warpTo
		},
		{'btn', id='warp', text='телепорт', width=70, onclick=warpTo},
		{'btn', id='cancel', text='закрыть', width=70, closeswindow=true}
	},
	oncreate = warpInit
}
	...
	controls = {
		{
			...
			width=280,
			columns={
				{text='Статистика', attr='name', width=0.6},
				{text='Значение', attr='value', width=0.3, enablemodify=true}
			},
			rows={xml='stats.xml', attrs={'name', 'id'}},
			onitemclick=selectStat,
			onitemdoubleclick=maxStat
		},
		{'txt', id='statval', text='', width=90},
		{'btn', id='set', text='выбор', width=90, onclick=applyStat},
		{'btn', id='close', text='закрыть', width=90, closeswindow=true}
	},
	oncreate = initStats
}
...
function saveLocation ()
	...
	if name ~= "" then
		...
		outputChatBox("Пожалуйста, введите имя для закладки")
	end
end
	...
	controls = {
		{
			...
			width=400,
			columns={
				{text='Название', attr='name', width=0.3},
				{text='Расположение', attr='zone', width=0.6}
			}
		},
		{'txt', id='bookmarkname', text='', width=186},
		{'btn', id='save current location', text='создание закладки', onclick=saveLocation, width=186},
		{'btn', id='delete selected location', text='удаление закладки', onclick=deleteLocation, width=186},
		{'btn', id='close', text='закрыть', closeswindow=true, width=186}
	},
...
function setPlayerPosition(x, y, z)
	...
	if elem then
		if getPlayerOccupiedSeat(g_Me) ~= 0 then
			errMsg('Только водитель транспорта может изменять его позицию!')
			return
		end
...
function updatePlayerBlips()
	...
	for elem,player in pairs(g_PlayerData) do
		if not player.gui.mapBlip then
			player.gui.mapBlip = guiCreateStaticImage(0, 0, 9, 9, elem == g_Me and '' or '', false, mapControl)
...
wndSetPos = {
	'wnd',
	text = 'Аэрофлот Б2Би. Добро пожаловать!',
	width = g_MapSide + 20,
	controls = {
		{'img', id='map', src='map.png', width=g_MapSide, height=g_MapSide, onclick=fillInPosition, ondoubleclick=setPosClick},
		{'txt', id='x', text='', width=80},
		{'txt', id='y', text='', width=80},
		{'txt', id='z', text='', width=80},
		{'btn', id='ok', text='выбрать', width=70, onclick=setPosClick},
		{'btn', id='cancel', text='закрыть', width=70, closeswindow=true},
		{'lbl', text='Нажмите ПКМ, чтобы закрыть это окно.', width=240}
	},
	oncreate = setPosInit,
...
function getPosCommand(cmd, playerName)
	...
	if playerName then
		...
		if not player then
			errMsg('Нет игрока с ником "' .. playerName .. '".')
			return
		end
		playerName = getPlayerName(player)		-- make sure case is correct
		sentenceStart = playerName .. ' сейчас '
	else
		player = g_Me
		sentenceStart = 'Вы '
	end
	...
	if vehicle then
		outputChatBox(sentenceStart .. 'в ' .. getVehicleName(vehicle), 0, 255, 0)
	else
		outputChatBox(sentenceStart .. 'пешком', 0, 255, 0)
	end
	outputChatBox(sentenceStart .. '(' .. string.format("%.5f", px) .. ' ' .. string.format("%.5f", py) .. ' ' .. string.format("%.5f", pz) .. ')', 0, 255, 0)
end
...
function setPosCommand(cmd, x, y, z, r)
	...
	if (message ~= "") then
		outputChatBox(message.."параметры не были введены. Используются ваши параметры "..message, 255, 255, 0)
	end
	...
	controls = {
		{
			...
			height=340,
			columns={
				{text='Название', attr='name'}
			},
			rows={xml='vehicles.xml', attrs={'id', 'name'}},
			onitemdoubleclick=createSelectedVehicle
		},
		{'btn', id='create', text='выбрать', width=70, onclick=createSelectedVehicle},
		{'btn', id='close', text='закрыть', width=70, closeswindow=true}
	}
}
...
function upgradesInit()
	local vehicle = getPedOccupiedVehicle(g_Me)
	if not vehicle then
		errMsg('Вы должны быть в автомобиле, чтобы устанавливать тюнинг!')
		closeWindow(wndUpgrades)
		return
	end
	if getPlayerOccupiedSeat(g_Me) ~= 0 then
		errMsg('Только водитель транспорта может устанавливать тюнинг!')
		closeWindow(wndUpgrades)
		return
	end
...
function selectUpgrade(leaf)
	setControlText(wndUpgrades, 'addremove', leaf.installed and 'снять' or 'установить')
end
...
function addRemoveUpgrade(selUpgrade)
	...
	if selUpgrade.installed then
		...
		setControlText(wndUpgrades, 'addremove', 'установить')
		...
		setControlText(wndUpgrades, 'addremove', 'снять')
	...
	controls = {
		{
			...
			height=340,
			columns={
				{text='Запчасть', attr='id', width=0.6},
				{text='Установлена', attr='installed', width=0.3, enablemodify=true}
			},
			onitemclick=selectUpgrade,
			onitemdoubleclick=addRemoveUpgrade
		},
		{'btn', id='addremove', text='установить', width=70, onclick=addRemoveUpgrade},
		{'btn', id='ok', text='закрыть', width=70, closeswindow=true}
	},
...
function paintjobInit()
	local vehicle = getPedOccupiedVehicle(g_Me)
	if not vehicle then
		errMsg('Вы должны быть в автомобиле, чтобы изменить его винил!')
		closeWindow(wndPaintjob)
		return
	end
	if getPlayerOccupiedSeat(g_Me) ~= 0 then
		errMsg('Только водитель транспорта может изменять его винил!')
		closeWindow(wndPaintjob)
		return
	end
	...
	controls = {
		{
			...
			height=150,
			columns={
				{text='Время', attr='name'}
			},
			rows={
				{name='Полночь',  h=0, m=0},
				{name='Ночь',     h=5, m=0},
				{name='Утро',     h=9, m=0},
				{name='Полдень',  h=12, m=0},
				{name='День',     h=15, m=0},
				{name='Вечер',    h=20, m=0},
				{name='Ночь',     h=22, m=0}
			},
		...
		{'btn', id='set', text='выбор', width=70, onclick=applyTime},
		{'btn', id='close', text='закрыть', width=70, closeswindow=true}
	},
	oncreate = timeInit
}
	...
	controls = {
		{
			...
			height=290,
			columns = {
				{text='Погода', attr='name'}
			},
			rows={xml='weather.xml', attrs={'id', 'name'}},
			onitemdoubleclick=applyWeather
		},
		{'btn', id='set', text='выбор', width=70, onclick=applyWeather},
		{'btn', id='close', text='закрыть', width=70, closeswindow=true}
	}
}
...
wndGameSpeed = {
	...
	controls = {
		...
		{'txt', id='speed', text='', width=73},
		{'btn', id='set', text='выбор', width=73, onclick=applyGameSpeed},
		{'btn', id='close', text='закрыть', width=73, closeswindow=true}
	},
...
function updateGUI(updateVehicle)
	...
	if updateVehicle then
		...
		if vehicle and isElement(vehicle) then
			...
			setControlText(wndMain, 'curvehicle', 'Пешком')
		end
	end
end
...
wndMain = {
	...
	width = 270,
	controls = {
		{'lbl', text='Текущий игрок'},
		{'br'},
		{'btn', id='kill', text='суицид', width=80, onclick=killLocalPlayer},
		{'btn', id='skin', text='скины', width=80, window=wndSkin},
		{'btn', id='anim', text='анимации', width=80, window=wndAnim},
		{'btn', id='weapon', text='оружие', width=80, window=wndWeapon},
		{'btn', id='clothes', text='одежда', width=80, window=wndClothes},
		{'btn', id='playergrav', text='гравитация', width=80, window=wndGravity},
		{'btn', id='warp', text='телепорт', width=80, window=wndWarp},
		{'btn', id='stats', text='статистика', width=80, window=wndStats},
		{'btn', id='bookmarks', text='закладки', width=80, window=wndBookmarks},
		{'br'},
		{'chk', id='jetpack', text='джетпак', width=80, onclick=toggleJetPack},
		...
		{'btn', id='setinterior', text='интерьеры', width=120, window=wndSetInterior},
		...
		{'btn', id='repair', text='починить', width=120, onclick=repairVehicle},
		{'btn', id='flip', text='перевернуть', width=120, onclick=flipVehicle},
		...
		{'lbl', text='Окружающая среда'},
		{'br'},
		{'btn', id='time', text='время', width=120, window=wndTime},
		{'chk', id='freezetime', text='заморозить', width=120, onclick=toggleFreezeTime},
		{'btn', id='weather', text='погода', width=120, window=wndWeather},
		{'btn', id='speed', text='скорость', width=120, window=wndGameSpeed}
	},
