...
g_RPCFunctions = {
	addPedClothes = { option = 'clothes', descr = 'Добавление одежды' },
	addVehicleUpgrade = { option = 'upgrades', descr = 'Установка тюнинга' },
	fadeVehiclePassengersCamera = true,
	fixVehicle = { option = 'repair', descr = 'Восстановление транспорта' },
	giveMeVehicles = { option = 'createvehicle', descr = 'Создание транспорта' },
	giveMeWeapon = { option = 'weapons.enabled', descr = 'Использование оружия' },
	givePedJetPack = { option = 'jetpack', descr = 'Использование джетпака' },
	killPed = { option = 'kill', descr = 'Совершение суицида' },
	removePedClothes = { option = 'clothes', descr = 'Снятие одежды' },
	removePedFromVehicle = true,
	removePedJetPack = { option = 'jetpack', descr = 'Снятие джетпака' },
	removeVehicleUpgrade = { option = 'upgrades', descr = 'Снятие тюнинга' },
	setElementAlpha = { option = 'alpha', descr = 'Изменение прозрачности' },
	setElementPosition = true,
	setElementInterior = true,
	setMyGameSpeed = { option = 'gamespeed.enabled', descr = 'Изменение скорости игры' },
	setMySkin = { option = 'setskin', descr = 'Изменение скинов' },
	setPedAnimation = { option = 'anim', descr = 'Изменение анимации' },
	setPedFightingStyle = { option = 'setstyle', descr = 'Изменение стиля боя' },
	setPedGravity = { option = 'gravity.enabled', descr = 'Изменение гравитации' },
	setPedStat = { option = 'stats', descr = 'Изменение статистики' },
	setTime = { option = 'time.set', descr = 'Изменение времени' },
	setTimeFrozen = { option = 'time.freeze', descr = 'Замораживание времени' },
	setVehicleColor = true,
	setVehicleHeadLightColor = true,
	setVehicleOverrideLights = { option = 'lights', descr = 'Включение фар' },
	setVehiclePaintjob = { option = 'paintjob', descr = 'Изменение аэрографии' },
	setVehicleRotation = true,
	setWeather = { option = 'weather', descr = 'Изменение погоды' },
	spawnMe = true,
	warpMe = { option = 'warp', descr = 'Телепортирование' }
}
...
function joinHandler(player)
	...
	if getOption('welcometextonstart') then
		outputChatBox('Server: Добро пожаловать на [Street-Drift] Server', player, 0, 255, 255)
		outputChatBox('Server: Докажи, что ты король дрифта!', player, 0, 255, 255)
		outputChatBox('Подсказка: Нажмите F1 для отображения/скрытия панели', player, 0, 255, 0)
	end
end
...
function warpMe(targetPlayer)
	...
	if not vehicle then
		...
		if getPedOccupiedVehicle(source) then
			...
			outputChatBox('Выйдите сначала из вашего транспорта.', source)
			return
		end
		...
		outputChatBox('В транспорте ' .. getPlayerName(targetPlayer) .. ' нет свободных мест.', source, 255, 0, 0)
	end
...
function giveMeWeapon(weapon, amount)
	...
	if table.find(getOption('weapons.disallowed'), weapon) then
		errMsg((getWeaponNameFromID(weapon) or tostring(weapon)) .. ' запрещено.', source)
	else
...
function giveMeVehicles(vehicles)
	...
	for i,vehID in ipairs(vehicles) do
		...
		if vehID < 400 or vehID > 611 then
			errMsg(vehID ..' неверное ID транспорта!', source)
		elseif not table.find(getOption('vehicles.disallowed'), vehID) then
			...
			errMsg(getVehicleNameFromModel(vehID):gsub('y$', 'ie') .. ' запрещено.', source)
		end
	end
end
...
function setPedGravity(player, grav)
	if grav < getOption('gravity.min') then
		errMsg(('Минимальная разрешенная гравитация: %.5f'):format(getOption('gravity.min')), player)
	elseif grav > getOption('gravity.max') then
		errMsg(('Максимальная разрешенная гравитация: %.5f'):format(getOption('gravity.max')), player)
	else
...
function setMyGameSpeed(speed)
	if speed < getOption('gamespeed.min') then
		errMsg(('Минимальная разрешенная скорость: %.5f'):format(getOption('gamespeed.min')), source)
	elseif speed > getOption('gamespeed.max') then
		errMsg(('Максимальная разрешенная скорость: %.5f'):format(getOption('gamespeed.max')), source)
	else
	...
	function(msg, type)
		if type == 0 then
			...
			if chatTime[source] and chatTime[source] + tonumber(get("*chat/mainChatDelay")) > getTickCount() then
				outputChatBox("Прекратите спамить в чат!", source, 255, 0, 0)
				return
			else
			...
			if get("*chat/blockRepeatMessages") == "true" and lastChatMessage[source] and lastChatMessage[source] == msg then
				outputChatBox("Прекратите повторять фразы!", source, 255, 0, 0)
				return
			else
...
addEventHandler('onServerCall', g_Root,
	function(fnName, ...)
		...
		elseif type(fnInfo) == 'table' then
			errMsg(fnInfo.descr .. ' запрещено.', source)
		end
	end
)
